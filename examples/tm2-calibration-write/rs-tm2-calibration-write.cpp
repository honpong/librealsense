    ...
    for (auto dev : ctx.query_devices()) {
        if (auto tm2 = dev.as<rs2::tm2>()) {
            for (auto sensor : tm2.query_sensors()) {
                ...
                std::cout << "Fisheye 1 to Fisheye 2 extrinsics : " << std::endl;
                print(video[0].get_extrinsics_to(video[1]));
                std::cout << "Fisheye 2 to Fisheye 1 extrinsics : " << std::endl;
                print(video[1].get_extrinsics_to(video[0]));
            }
