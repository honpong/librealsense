...
namespace rs2
{
    ...
    public:
        ...
        * \return true if the operation is complete successfully.
        */
        bool set_intrinsics(int sensor_id, const rs2_intrinsics& intr)
        {
            rs2_error* e = nullptr;
            int status = rs2_set_tm2_intrinsics(_dev.get(), sensor_id, &intr, &e);
            error::handle(e);
            return status == 0;
        }
        ...
        */
        bool set_extrinsics(rs2_stream stream_type, int stream_index, const rs2_extrinsics& extr)
        {
            rs2_error* e = nullptr;
            int status = rs2_set_tm2_extrinsics(_dev.get(), stream_type, stream_index, &extr, &e);
            error::handle(e);
            return status == 0;
        }
        ...
        * \return true if the operation is complete successfully.
        */
        bool set_motion_device_intrinsics(int sensor_id, const rs2_motion_device_intrinsic& intr)
        {
            rs2_error* e = nullptr;
            int status = rs2_set_tm2_motion_device_intrinsics(_dev.get(), sensor_id, &intr, &e);
            error::handle(e);
            return status == 0;
        }
        ...
        */
        ...
        {
            rs2_error* e = nullptr;
            int status = rs2_reset_tm2_to_factory_calibration(_dev.get(), &e);
            error::handle(e);
            return status == 0;
        }
    };
