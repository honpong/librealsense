...
/**
* set frame compressed size
* \param[in] frame      handle returned from a callback
* \param[out] error     if non-null, receives any error that occurs during this call, otherwise, errors are ignored
* \return               compressed size if compressed, and -1 otherwise
*/
void rs2_set_frame_compressed_size(const rs2_frame* frame, int size, rs2_error** error);
...
* \return               compressed size if compressed, and -1 otherwise
*/
void rs2_set_frame_format(const rs2_frame* frame, rs2_format format, rs2_error** error);
